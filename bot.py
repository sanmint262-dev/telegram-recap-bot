...
def check_daily_limit(update: Update):
...
def use_daily_limit(update: Update):

    # =====================================================
    # ADMIN DOES NOT CONSUME QUOTA
    # =====================================================
...
async def send_long_message(
    ...
    # -----------------------------------------------------
    # Prefer newline / paragraph breaks
    # -----------------------------------------------------
...
async def start(
    ...
    if is_admin(update):
        ...
        quota_text = (
            "👑 Admin Account\n"
            "♾️ Daily MP3 Limit: Unlimited"
        )

    else:

        _, used = check_daily_limit(
            update
        )
...
async def handle_audio(
    ...
    allowed, used = check_daily_limit(
        update
    )
    ...
    if not allowed:
        ...
        await update.message.reply_text(
            ...
            f"📊 အသုံးပြုပြီး: {used}/{DAILY_FREE_LIMIT}\n"
    ...
    if is_admin(update):
        ...
        new_used = get_user_usage(
            update.effective_user.id
        )["count"]

        quota_status = (
            f"📊 Today: "
            f"{new_used}/{DAILY_FREE_LIMIT}"
        )
